...
const DriverDaylyEarningsComponents = ({commandDailyList}) => {
    ...
    const getRidesAmount = () => {
        var finalAmt = 0
        var amount = 0
        commandDailyList.forEach(element => {
    ...
    return (
        <View style = { styles.container } >
            <View style = { styles.btnComp } >
                ...
                <View style = { styles.btnWrapper } >
                    <Text style = {[ styles.textEuro, { color: colors.primary } ]} > { commandDailyList.length } </Text>
                    <Text style = { styles.btnText } > { commandDailyList.length > 1 ? t('RidesMade') : t('RideMade') } </Text>
                </View>
            </View>
            <View style = { styles.formTrips } >
                <Text style = {[ styles.btnText, { padding: 5 } ]} > { t('TodaysRides') } </Text>
                <ScrollView>
                    {
                        commandDailyList.map((item, index)=> {
...
export default DriverDaylyEarningsComponents
